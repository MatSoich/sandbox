...
class SchemaElement:
    ...
    def getLabel(self):
        if self.verbose_label is None and self.label is None:
            assert self.uri in [
                'http://www.xbrl.org/2003/instance', 'http://www.w3.org/2001/XMLSchema']
...
def check_taxonomy():
    for date in ['2013-08-31', '2015-03-31', '2016-02-29', '2017-02-28', '2018-02-28', '2019-11-01']:  # '2014-03-31'
        xsd_path = '%s/data/EDINET/taxonomy/%s/taxonomy/jppfs/%s/jppfs_cor_%s.xsd' % (
            root_dir, date, date, date)
...
def read_company_dic():
    ...
    lines = read_csv_file(
        root_dir + '/data/EDINET/EdinetcodeDlInfo.csv', 'shift_jis')

    # 最初の2行の見出しは取り除く。
    lines = lines[2:]

    # 会社情報の辞書。キーはEDINETコード。値は上場区分, 会社名, 業種(日本語), 業種(英語)
    company_dic = dict((x[0], {'listing': x[2], 'company_name': x[6], 'category_name_jp': x[10],
                       'category_name': category_name_dic[x[10]]}) for x in lines if 11 <= len(x))
        ...
        if tag_name == "loc":
            ...
            if 'label' in attr and 'role' in attr:
                if attr['role'] in [label_role, verboseLabel_role]:
                    resource_dic[attr['label']] = {
                        'role': attr['role'], 'text': el.text}
...
def setChildren(inf, ctx: ContextNode):
    ...
    for item in ctx.node_items:
        ...
        if not item.schema.sorted:
            ...
            item.schema.calcTo = sorted(
                item.schema.calcTo, key=lambda x: x.order)
...
def get_schema_label_path(inf, ns_uri):
    ...
    if ns_uri.startswith("http://disclosure.edinet-fsa.go.jp/taxonomy/"):
        ...
        xsd_path = (taxonomy_tmpl % yyyymmdd) + name_space + \
            '/' + yyyymmdd + '/' + file_name
        label_path = (taxonomy_tmpl % yyyymmdd) + name_space + '/' + \
            yyyymmdd + '/label/' + name_space + "_" + yyyymmdd + '_lab.xml'
        ...
        xsd_path = "%s/data/IFRS/IFRST_%s/full_ifrs/full_ifrs-cor_%s.xsd" % (
            root_dir, yyyymmdd, yyyymmdd)
        assert os.path.exists(xsd_path)

        yyyy = yyyymmdd.split('-')[0]
        label_path = '%s/data/IFRS/ja/Japanese-Taxonomy-%s/full_ifrs/labels/lab_full_ifrs-ja_%s.xml' % (
            root_dir, yyyy, yyyymmdd)
        ...
        assert ns_uri in ["http://www.xbrl.org/2003/instance",
...
def makeContextNode(inf, ctx: Context):
    ...
    for dimension_schema, member_schema in zip(ctx.dimension_schemas, ctx.member_schemas):
        ...
        # スキーマが同じメンバーを探す。
        leaf_node = find(
            x for x in dimension.members if x.schema == member_schema)
        ...
        if text is None:
            if 'nillable' in schema.el.attrib and schema.el.attrib['nillable'] == 'true':
                ...
                if inline_xbrl_path is not None:
                    ...
                    print(xbrl_path)
                print('\ttext is none', uri, tag_name,
                      schema.type, schema.el.attrib)
...
def cased_path(path):
    ...
    for fname in path.lower().split('/'):
        rel_path += '/' + \
            find(x for x in os.listdir(abs_path + rel_path) if x.lower() == fname)
...
def read_public_doc(inf, category_name, public_doc, reports):
    ...
    for local_xsd_path_obj in Path(inf.cur_dir).glob("*.xsd"):
        ...
        # スキーマファイルの内容を読む。
        ReadSchema(inf, True, local_xsd_path, ET.parse(
            local_xsd_path).getroot(), local_xsd_dic)

        # 名称リンクファイルのパス
        local_label_path = local_xsd_path[:len(
            local_xsd_path) - 4] + "_lab.xml"
        if os.path.exists(local_label_path):
            ...
            loc_dic = {}
            # 名称リンクファイルの内容を読む。
            ReadLabel(ET.parse(str(local_label_path)).getroot(),
                      local_xsd_dic, loc_dic, resource_dic)
            label_cnt += 1
        ...
        if os.path.exists(local_cal_path):
            ...
            # 計算リンクファイルの内容を読む。
            readCalcSub(inf, ET.parse(local_cal_path).getroot(),
                        local_xsd_dic, locs, arcs)
    ...
    htm_paths = [str(x).replace('\\', '/')[web_path_len:]
                 for x in Path(inf.cur_dir).glob("*.htm")]
...
def make_public_docs_list(cpu_count, company_dic):
    ...
    for public_doc in Path(report_path).glob("**/XBRL/PublicDoc"):
        ...
        if edinet_code in edinet_code_dic:
            ...
            edinet_code_dic[edinet_code] = {
                end_date: (filing_date, public_doc)}

        # カテゴリー名
        company = company_dic[edinet_code]
        category_name = company['category_name']
        company_obj = {'edinet_code': edinet_code,
                       'company_name': company['company_name']}

        category = find(
            x for x in category_companies if x['category_name'] == category_name)
        if category is None:

            # カテゴリー別の会社のリストに追加する。
            category_companies.append(
                {'category_name': category_name, 'companies': [company_obj]})
        ...
        for category in category_companies:
            category['companies'] = sorted(
                category['companies'], key=lambda x: x['company_name'])
...
def readXbrlThread(cpu_count, cpu_id, edinet_code_dic, progress, company_dic):
    ...
    inf.logf = open('%s/data/log-%d.txt' %
                    (root_dir, cpu_id), 'w', encoding='utf-8')
    ...
    for edinet_code, end_date_dic in edinet_code_dic.items():
        ...
        inf.local_top_context_nodes = sorted(
            inf.local_top_context_nodes, key=lambda x: period_names_order.index(x.period))

        top_nodes = [
            x for x in inf.local_top_context_nodes if x.period in inf.period_end_dates]
        ...
        json_data = {'reports': reports,
                     'top_nodes': top_nodes, 'version': '1.1'}
...
if __name__ == '__main__':
    ...
    readXbrlThread(cpu_count, cpu_id,
                   public_docs_list[cpu_id], progress, company_dic)
